...
export function AdminSidebar() {
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          'fixed top-0 left-0 h-full w-64 bg-white border-r z-50 transform transition-transform duration-200 flex flex-col',
          'lg:translate-x-0 lg:static',
          isMobileOpen ? 'translate-x-0' : '-translate-x-full'
        )}
      >
        {/* Logo */}
        <div className="h-14 flex items-center gap-2 px-4 border-b flex-shrink-0">
          <Building2 className="h-6 w-6 text-primary" />
        ...
        <div className="p-4 border-t space-y-1 flex-shrink-0">
          ...
          >
            ...
            <span className="truncate">사이트 보기</span>
          </Link>
          <button
            onClick={handleLogout}
            className="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-100 transition-colors w-full text-left"
          >
